...
app = FastAPI(
    ...
    description="API de prédiction, monitoring et alerting du churn client",
    version="1.0.0",
    docs_url="/docs",
    redoc_url="/redoc",
)
...
def log_drift_to_insights(drift_results: dict):
    ...
    percentage = round((drifted / total) * 100, 2) if total else 0.0
    ...
    try:
        ...
        return {
            ...
            "features_drifted": sum(
                1 for r in results.values() if r.get("drift_detected")
            ),
        }
